...
def get_theta(speed, angle, time_step):

    v = speed_to_velocity(speed)
    distance = v * time_step
    heading_angle = np.radians(90-angle)
...
##    run_action('stop')
    ...
    while True:
        ...
        delta_theta = get_theta(speed_circle, int(angle_circle), timestep_circle)

        theta_now = theta_now + delta_theta
        print(f'Theta = {theta_now}')

        if theta_now > np.pi:          
          
            angle_circle = angle_circle + 10
            theta_now = 0

        query_img = QueryImage(HOST)
        qrcodes = query_img.getQRcode()
        if not qrcodes:
            print('Travelling blindly')
        else:
            print('Barcode found')
            qrcode = qrcodes[0]
            (x, y, w, h) = qrcode.rect
            ...
            if w > 110:                
                run_action('stop')
                reset_car_state()
                #test([0, 1, np.radians(90)], hyperparams, waypoint="NIL",final_width=125)
                angle_circle = 20
                theta_now = 0
                
                run_fwturn(angle_change)
                run_speed(str(speed_change))
                run_action('forward')

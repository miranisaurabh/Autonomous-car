...
def create_voronoi(grid):

    voronoi = np.zeros((dim, dim))
    voronoi.fill(dim)
    voronoi[24:36, 24:36] = 0
        ...
        for j in range(dim):
            ...
            for m in range(dim):
                ...
                for n in range(dim):
                    ...
                    if 24 < i < 36 and 24 < j < 36:
                        continue
                    if grid[m,n] == 1:
                        distance = np.sqrt((m-i)**2 + (n-j)**2)
                        if distance < voronoi[i,j]:
                            voronoi[i,j] = distance
    voronoi = np.floor(voronoi)

    return grid

def get_voronoi_vertices(voronoi):

    max_val = np.amax(voronoi)
    reuslt = np.where(voronoi == max_val)
    vertices = np.array([0,0])
    for i in range(len(reuslt[0])):

        if i==0:
            vertices[0] = np.array([reuslt[0][0],reuslt[1][0]])
        else:
            vertices = np.vstack(vertices,[reuslt[0][i],reuslt[1][i]])

    return vertices

def get_nearest_vertex(vertices,point):

    min_d = 60.0 #dim
    for i in range(len(vertices)):

        x = vertices[i,0] - point[0]
        y = vertices[i,1] - point[1]
        d = np.sqrt(x**2 + y**2)
        if d < min_d:
            nearest_vertex = vertices[i,:]
            nearest_id = i

    return nearest_vertex,nearest_id


def get_greedy_path(vertices,start,goal):

    nearest_vertex,nearest_id = get_nearest_vertex(vertices,start)
    path = nearest_vertex
    count = nearest_id
    while True:

        x_goal1 = vertices[count+1,0] - goal[0]
        y_goal1 = vertices[count+1,1] - goal[1]
        d_goal1 = np.sqrt(x_goal1**2 + y_goal1**2)

        x_goal2 = vertices[count-1,0] - goal[0]
        y_goal2 = vertices[count-1,1] - goal[1]
        d_goal2 = np.sqrt(x_goal2**2 + y_goal2**2)

        x_goal = vertices[count,0] - goal[0]
        y_goal = vertices[count,1] - goal[1]
        d_goal = np.sqrt(x_goal**2 + y_goal**2)

        if d_goal < d_goal1 or d_goal < d_goal2:
            path = np.vstack(path,goal)
            return path
        elif d_goal1 < d_goal2:
            path = np.vstack(path,vertices[count+1,:])
            count = count + 1
        else:
            path = np.vstack(path,vertices[count+2,:])
            count = count - 1
...
if __name__ == '__main__':
    ...
    dim = 60 #Each cell is of 5cms. 3mx3m reduces to 60x60 cells
    # grid is the occupancy map
    ...
    grid[24:36, 24:36] = 1.0 #Obstacle

    voronoi = create_voronoi(grid)
    vertices = get_voronoi_vertices(voronoi)
    start = [0,0]
    goal1 = [2,2]
    goal2 = [2.6,1.5]
    path = get_greedy_path(vertices,start,goal1)

    for i in range(len(path)):

        x = path[i+1].y - path[i].y
        y = path[i+1].x - path[i].x
        d = np.sqrt(x**2 + y**2)
        ...
        if i+2 < len(path):
            check = np.isin(voronoi,path[i+2])
            check_new = check.T
            if check_new[0] & check_new[1]:
                x = path[i+2].y - path[i+1].y
                y = path[i+2].x - path[i+1].x
                rel_pose = round(np.arctan2(y,x) - pose,2)

        pose = np.arctan2(y,x)
        final_pose = [path[i+1].x,path[i+1].y]
        #test([rel_x, rel_y, rel_pose], hyperparams,final_pose)

    path = get_greedy_path(vertices,goal1,goal2)
